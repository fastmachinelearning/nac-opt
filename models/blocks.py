...
class ConvBlock(torch.nn.Module):
    def __init__(self, channels, kernels, acts, norms, img_size):
        ...
        for i in range(len(kernels)):
            self.layers.append(
                nn.Conv2d(channels[i], channels[i + 1], kernel_size=kernels[i], stride=1, padding=0)
    ...
    def __init__(self, channels, kernels, acts, norms, img_size, bit_width=8):
        ...
        for i in range(len(kernels)):
            conv = qnn.QuantConv2d(
                channels[i], channels[i + 1], kernel_size=kernels[i], stride=1, padding=0, weight_bit_width=bit_width
            )
            ...
            if acts[i] is not None:
                act_layer = (
                    acts[i]
                    if isinstance(acts[i], nn.Module)
                    else qnn.QuantIdentity(bit_width=bit_width, return_quant_tensor=True)
                )
...
class QAT_CandidateArchitecture(torch.nn.Module):
    def __init__(self, Blocks, MLP, hidden_channels, input_channels=1, bit_width=8):
        ...
        )  # initial layer to initialize quantization
        self.conv = qnn.QuantConv2d(
            input_channels,
            hidden_channels,
            kernel_size=(3, 3),  # initial projection layer quantized
            stride=(1, 1),
